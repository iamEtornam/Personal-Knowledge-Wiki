...
Trigger the correct operation when the user says any of the following:

| What the user says | Operation |
|--------------------|-----------|
| "ingest", "process my data", "convert my files" | → **ingest** |
| "absorb", "compile wiki", "build wiki", + optional date range | → **absorb** |
| Any question about themselves, their life, projects, people | → **query** |
| "cleanup", "audit", "fix the wiki" | → **cleanup** |
| "breakdown", "find missing articles", "expand" | → **breakdown** |
| "rebuild index", "rebuild backlinks" | → **rebuild-index** |
| "reorganize", "restructure" | → **reorganize** |
| "status", "stats", "how big is the wiki" | → **status** |
...
Convert source data from `data/` into individual `.md` files in `raw/entries/`. Write a Python script `ingest.py` to do this. This step is mechanical — no LLM intelligence needed.
...
### Reclassification (when asked to reorganize)
